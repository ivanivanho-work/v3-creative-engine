...
- **Flag any prompt that contains text matching the pattern "(consistent with ref_...)" or any similar internal reference marker.** These IDs mean nothing to a generation model. The fix instruction should be to replace the marker with the subject's physical description from the corresponding `canonical_description` (e.g. replace "(consistent with ref_aud_korea_male_25-34_racing_drone)" with "a sleek, black and red high-performance racing drone with multiple rotors and carbon fiber textures"). The dependency is already recorded in `ref_dependencies`.
- **Flag any prompt that contains a bare ref_id** (e.g. "ref_aud_korea_female_25-34_eiffel_tower" appearing anywhere in the prompt text, with or without parentheses or the word "consistent"). Any string matching the pattern `ref_[a-z_]+` inside a prompt field is an internal ID that must not appear there. Fix instruction: replace it with the subject's physical description from `canonical_description`.
- **Flag any prompt that references another scene by ID** (e.g. "the one from scene_01", "as in scene_02", "look distinctly like the one from scene_01"). Scene IDs are internal schema identifiers that mean nothing to a generation model. Fix instruction: replace the scene reference with the subject's physical description sourced from the relevant `canonical_description`.
