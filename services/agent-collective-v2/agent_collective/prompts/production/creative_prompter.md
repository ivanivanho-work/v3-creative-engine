...
3. **Write the generation prompt.** Each prompt must include:
   ...
   - **Persona injection:** If the element references a recurring element (via `recurring_element_ref`), inject the full `canonical_description` from the corresponding reference_image directly into the prompt text. Do NOT write "(consistent with [ref_id])" or any similar internal marker in the prompt - the generation model does not know what these IDs are. The dependency is already recorded in the `ref_dependencies` field.
