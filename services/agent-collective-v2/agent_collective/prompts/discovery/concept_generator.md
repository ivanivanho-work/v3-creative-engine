...
**If `campaign_constraints.campaign_topic` is set:** ALL THREE concepts MUST be anchored in the stated topic territory — this behaves the same as `featured_tool` locking. Topic diversity is replaced by diversity in *how* the topic is approached: different content angles, different audience segments, different emotional hooks, all within the stated topic. Use `content_insights` to find the matching or closest content category. Check `content_trends` for any `intel-hub-brief-*` entries whose topic maps to this territory — if found, use them as creative departure points. **Important:** the trend defines the *content territory*, not the featured tool. If the trend's native feature (e.g. a GenAI tool) has `recommendation_strength: "low"`, do not use it — instead, choose the highest-rated available tool that can serve the same creative territory (e.g., if the trend is about pet meme audio, Add Audio or Templates can serve the content; if it's about visual transformation, Effects can serve it). Cite the Intel Hub trend in the rationale as validation of the content territory.
...
- **Campaign topic (if set):** If `campaign_constraints.campaign_topic` is not null, ALL THREE concepts MUST be anchored in that topic territory. Diversity shifts to creative angle, audience segment, and featured tool — not topic.
...
**State reads:**
- `campaign_constraints` - Optional user-specified constraints. Check before generating concepts. Fields: `featured_tool` (lock all concepts to this tool if set) and `campaign_topic` (anchor at least one concept to this topic if set).
