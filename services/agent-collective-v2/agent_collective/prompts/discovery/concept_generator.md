...
- **Campaign topic (if set):** If `campaign_constraints.campaign_topic` is not null, at least one concept MUST be anchored in that topic territory. The remaining concepts should explore different territories — do not make all three about the same topic.
