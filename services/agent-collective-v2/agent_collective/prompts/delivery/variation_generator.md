...
* NEVER write "(consistent with ref_xxx)" or any internal reference ID inside a generation prompt. These IDs mean nothing to a generation model. Describe the subject using its canonical description only. Reference dependencies are recorded in `ref_dependencies`, not in prompt text.  
