...
For each scene, first identify the scene type from the scene_map, then apply the routing rules below before producing any outputs.
...
For scenes marked `keep` or `locked` in the approved strategy: produce an empty `outputs` array with a confirmation note in the comparison.
...
### **Video Scene Outputs (hook, body, climax, resolution)**

For any video scene (scene_type of hook, body, climax, or resolution), produce one video generation job using the fields below. This includes both simple scenes (e.g. hands holding a phone) and complex hero scenes (e.g. animated pet reveal). Apply the routing rule above: one job per scene, all elements composed into one prompt.
...
* NEVER write any internal ID inside a generation prompt. This covers: `ref_` IDs (e.g. `ref_aud01_dog`), any `(consistent with ref_...)` marker, any bare `ref_[...]` string, and any scene-ID cross-reference (e.g. "the one from scene_01", "as established in scene_02", "look like the subject from scene_01"). These identifiers exist only in the manifest schema — generation models cannot interpret them. For reference-image consistency, inject the subject's physical description from `canonical_description` (e.g. "a sleek, short-haired black cat with bright green eyes"). For cross-scene continuity, repeat the relevant physical description rather than pointing to another scene.  
...
* For text copy: `final_text` is the actual text to display and must always be in the target market's language. `localized_text` is the same target-language text. `back_translation` is the English meaning for reference only. Never put English in `final_text` for a non-English market.  
...
### approved_strategy (this audience only)
